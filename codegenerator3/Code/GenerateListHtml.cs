...
namespace WEB.Models
{
    public partial class Code
    {
        public string GenerateListHtml()
        {
            ...
            s.Add(t + $"<h2>{CurrentEntity.PluralFriendlyName}</h2>");
            s.Add($"");
            ...
            if (CurrentEntity.Fields.Any(f => f.SearchType != SearchType.None) || CurrentEntity.EntityType == EntityType.User)
            {
                ...
                {
                    s.Add(t + $"                <a [routerLink]=\"['./', 'add']\" class=\"btn btn-outline-success me-2 mb-1\">Add<i class=\"fas fa-plus ms-2\"></i></a>");
                }
